...
class TemperaturePredictor(GridLayout):
    ...
    def tick(self,dt):
        ...
        self.count_time='{:.2f} secs'.format(current_time)
        self.ids.temp_read.text= '{:.3f} deg'.format(temp)
        ...
        if current_time>self.time_in +15 and not self.predicted:
            temp_pred= self.PredictFunction(self.temp_list, self.time_list)
